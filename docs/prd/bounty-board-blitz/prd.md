# PRD: Bounty Board Blitz

## 0. One-Screen Summary (Problem / Why / How)

- Problem it solves: 组织者发布任务成本高，玩家接单流程不清晰，履约效率低。
- Why this matters now: 任务分发依赖人工沟通，效率低。
- How it solves it (3-step mechanism):
  1. 配置任务目标、时限、奖励。
  2. 队伍接单并执行任务。
  3. 自动/人工核验后执行结算。
- Expected player/business outcome: 任务从发布到结算闭环稳定。

## 0.1 Before vs After

| Dimension | Before | After |
|---|---|---|
| Player clarity | 任务分发依赖人工沟通，效率低。 | 任务从发布到结算闭环稳定。 |
| Player outcome | 目标不清、协作效率受限 | 任务从发布到结算闭环稳定。 |
| Business outcome | 复玩率与营收闭环不稳定 | 可验证的玩法闭环与结算闭环 |

## 1. Document Control

- Product Name: Bounty Board Blitz
...
- Related Docs:
  - SPEC: docs/spec/bounty-board-blitz/spec.md (planned)
  - TODO: docs/todo/bounty-board-blitz/todo.md (planned)
  - Test Plan: docs/test-plan/bounty-board-blitz/test-plan.md (planned)

## 2. Background and Goal

- Problem Statement:
  - 需要将 Frontier 核心循环（生存、协作、竞争、经济）转化为可快速开局、可复玩、可结算的街机玩法。
  - 组织者发布任务成本高，玩家接单流程不清晰，履约效率低。
- Target Users:
  - Frontier 玩家（个人与小队）
  - 联盟组织者 / 社区活动发起者
- Product Goal:
...
|---|---|---|---|
| S-001 | 任务发布者 | 快速发布可验收任务并自动结算 | 创建复杂、验收成本高 |
| S-002 | 接单队伍 | 快速挑选任务并高效履约 | 规则不清、收益不确定 |

## 2.3 Success Metrics and Guardrails

- North Star Metric: 任务履约完成率
- Primary metrics:
  - M-001: 任务发布到被接单中位耗时 <= 3 分钟
  - M-002: 任务争议率 < 5%
- Guardrail metrics:
  - G-001: 无效证据拦截率 = 100%
  - G-002: 重复结算事件数 = 0
...
|---|---|---|
| P0 | Hackathon 72h | 任务发布、接单、核验、结算 |
| P1 | Post-hackathon 2-4 weeks | 任务模板市场、自动仲裁策略 |
...
- External dependencies (API/network/team): 证据源查询、仲裁流程
- Critical assumptions: 证据格式可标准化

## 4. Feature List

| Feature ID | Feature Name | Priority (P0/P1/P2) | Value Hypothesis | Status |
|---|---|---|---|---|
| F-009 | Bounty Board Blitz Core Gameplay | P0 | 任务市场化能提升玩家行为组织效率并沉淀可复用运营资产。 | Planned |
...
### F-009 Bounty Board Blitz Core Gameplay

- User Story: 作为组织者，我希望快速发布任务并让玩家协作完成。
- Trigger: 发布者创建任务单。
- Preconditions: 奖励资金已托管。
- Main Flow:
1. 配置任务目标、时限、奖励。
2. 队伍接单并执行任务。
3. 自动/人工核验后执行结算。
...
- Data Rules:
- 同一任务同一时间窗口默认只允许一个队伍占有。
- 结算证据需可复核且版本化。
- Error Handling:
  - 非法状态迁移、重复结算、重复领奖一律拒绝。
- Acceptance Criteria:
  - AC-001: 任务从发布到结算闭环稳定。
  - AC-002: 任务履约记录可用于信誉分更新。

### Step-by-step Gameplay User Stories

1. 作为发布者，我想几步内创建任务并锁定奖励。
2. 作为玩家，我想筛选适合我队伍能力的任务。
3. 作为接单队伍，我想清楚知道验收标准。
4. 作为执行者，我想提交证据后快速获得结果。
5. 作为参与方，我想在结算后看到信誉和收益变化。

### Core Loop Mapping

- Entry: 浏览任务墙并选单。
- Action: 执行任务并提交证据。
- Feedback: 任务状态、核验进度、争议提示。
- Reward: 奖励发放、信誉提升、解锁高阶任务。
- Re-engagement trigger: 任务链与每日挑战刷新。
...
- Monetization touchpoint: 任务发布手续费 + 高级任务位。
- Pricing / fee logic: 基础发布免费，高曝光任务收费。
- Sink/faucet impact: 手续费作为平台 sink，奖励从托管池发放。
- Anti-abuse rule: 防刷发布和伪造证据风控。
...
- 2. 任务是什么？
  - Core mission statement: 发布/接取任务并完成可核验证据提交，推动任务闭环结算。
  - Completion condition: 任务验收通过并按规则获得奖励与信誉。
  - Failure condition: 证据无效、超时或争议未解决导致结算失败。
- 3. 玩家完成任务后，如何分配？
...
- Move module impacts: TaskBoard、TaskClaim、EvidenceProof、TaskSettlement。
- On-chain/off-chain boundary: 链上记录任务状态和结算；链下处理证据预处理。
- Must follow testing standard: docs/sui-devnet-testing-standard.md
- Security and abuse considerations:
  - Replay: 证据提交绑定 task_id + submit_nonce。
  - Double settlement: 同任务同证据不可重复结算。
  - Privilege checks: 仲裁入口权限隔离。

### Contract Feasibility Review (Based on docs/all_contracts.move.txt)

- Feasibility verdict: Medium（玩法逻辑需新模块）
- Directly reusable contract primitives: inventory/storage_unit 可承载奖励托管，access_control 可承载任务发布权限。
- Missing capabilities / required new modules: 现有合约没有任务板、接单、证据仲裁标准，需要 TaskBoard/TaskClaim/Evidence 新模块。
- P0 delivery boundary: 先做结构化任务模板 + 托管结算；复杂仲裁和信誉联动放到 P1。
