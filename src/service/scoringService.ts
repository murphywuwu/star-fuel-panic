...
function buildMemberScores(team: TeamState, events: ScoreEvent[]): MemberScoreLine[] {
...
export const scoringService = {
  buildScoreBoard(matchId: string, teams: TeamState[], events: ScoreEvent[]): ScoreBoard {
    const teamRows: TeamScoreLine[] = teams.map((team) => {
      const members = buildMemberScores(team, events);
