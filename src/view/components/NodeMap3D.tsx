...
import { useNodeMapViewController } from "@/controller/useNodeMapViewController";
import type { NetworkNode } from "@/types/node";
import type { SolarSystemMapPoint } from "@/types/solarSystem";
...
export function NodeMap3D({ className = "", onNodeClick }: NodeMap3DProps) {
  const { nodeMap, view } = useNodeMapViewController();
  const {
    layer,
    renderMode,
    overviewLevel,
