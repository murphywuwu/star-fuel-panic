...
import { rejectPlanningTeamApplication } from "@/server/planningTeamRuntime";
...
export async function POST(
  ...
  const mutation = await prepareSignedMutation(
    request,
    `POST:/api/planning-teams/${id}/applications/${applicationId}/reject`,
    body,
    {
      walletAddress: String(body.walletAddress ?? ""),
      signature: String(body.signature ?? ""),
      message: String(body.message ?? ""),
      scope: "FuelFrogPanic:reject-planning-team-application",
  ...
  const result = rejectPlanningTeamApplication({
    teamId: id,
    applicationId,
    walletAddress: String(body.walletAddress ?? ""),
  ...
  if ("ok" in result.body && result.body.ok === false) {
    return finalizeMutation(`POST:/api/planning-teams/${id}/applications/${applicationId}/reject`, mutation.mutation, {
